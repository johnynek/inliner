...
object SpecializedOption {
  import scala.language.implicitConversions

  /** An implicit conversion that converts an option to an iterable value
   */
  implicit def specializedOption2Option[A](xo: SpecializedOption[A]): Option[A] = xo match {
    case SpecializedNone => None
    case SpecializedSome(s) => Some(s)
  }

}

sealed trait SpecializedOption[@specialized(Int, Long, Double, Char, Boolean/*, AnyRef*/) +A] {
...
case class SpecializedSome[@specialized(Int, Long, Double, Char, Boolean/*, AnyRef*/) +A](g: A) extends SpecializedOption[A] {
  def isEmpty = false
  def get: A = g
}
case object SpecializedNone extends SpecializedOption[Nothing] {
  def get: Nothing = sys.error("Not present")
  def isEmpty = true
}
/**
 ...
 */
class InlineArraySpecializedOption[T](o: TraversableOnce[T]) {
  import InlineArraySpecializedOption._

  def find(fn: T => Boolean): SpecializedOption[T] = macro findMethod[T]
...
object InlineArraySpecializedOption {
  implicit class ToInlineArraySpecializedOption[T](val o: Array[T]) extends AnyVal {
    @inline def specializedInline: InlineArraySpecializedOption[T] = new InlineArraySpecializedOption(o)
  }

  def find[T](ts: Array[T])(fn: T => Boolean): SpecializedOption[T] = macro findMacro[T]

  private[this] def findTree[T](c: Context)(ts: c.Tree, fn: c.Expr[T => Boolean])(implicit T: c.WeakTypeTag[T]): c.Expr[SpecializedOption[T]] = {
    import c.universe._
    //println(showRaw(fn))
    ...
    val tree = q"""{
      var $res: _root_.com.github.johnynek.inliner.SpecializedOption[$T] = _root_.com.github.johnynek.inliner.SpecializedNone
      var $idx = 0
      val $len = $ts.length
      while($idx < $len) {
        val $arg = $ts($idx)
        if($newTree) { $res = _root_.com.github.johnynek.inliner.SpecializedSome($arg); $idx = $len }
        $idx += 1
      }
      $res
    }"""
    //println(tree)
    c.Expr[SpecializedOption[T]](tree)
  }

  def findMacro[T](c: Context)(ts: c.Expr[Array[T]])(fn: c.Expr[T => Boolean])(implicit T: c.WeakTypeTag[T]): c.Expr[SpecializedOption[T]] =
    findTree(c)(ts.tree, fn)

  def findMethod[T](c: Context)(fn: c.Expr[T => Boolean])(implicit T: c.WeakTypeTag[T]): c.Expr[SpecializedOption[T]] =
